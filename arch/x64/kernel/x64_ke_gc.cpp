...
//
// @(#)$Id: //proj/evcl3/mainline/arch/x64/kernel/x64_ke_gc.cpp#3 $
//
// Description:
...
void Gc::beforeUpdateThread(Thread* pThread)
{
    Int* pStart = reinterpret_cast<Int*>(
        pThread->ToInt() - X64::X64Mach::Thread_Lead_Extra );
...
bool Gc::updateFrame(Thread* pThread, Frame* pFrameIn)
{
    ...
    *reinterpret_cast<UInt*>(pFrame->m_pvRA) =
    ...
    if (NULL == pGcDesc)
    {
        ...
        GC_FORMAT(L"  Frame=#x~X cb=#x~X ofsAnnot=#x~X ofsGcMap=#~X~%",
        ...
        GC_FORMAT(L"  Gc Map~%");
        foreach (GcMap::Enum, oEnum, &oGcMap)
        {
            GC_FORMAT(L"    +#x~X #x~X~%",
                Fixnum::Encode(oEnum.GetOffset()),
                Fixnum::Encode(*oEnum.GetDesc()) );
        } // for each desc
    ...
    uint cBits;
    {
        const uint kRegs = lengthof(X64::X64Mach::k_rgnGcMapIdx2Reg) - 1;
        switch (nGcDesc & 3)
        {
        case GcDesc_General:
            nGcDesc >>= kRegs + 2;
...
uint GcMap::Skip(uint32 nGcDesc)
{
    ...
    case GcDesc_General << 1:
        ...
        return (lengthof(X64::X64Mach::k_rgnGcMapIdx2Reg) - 1) + 3;
