...
;;;
;;; @(#)$Id: //proj/evcl3/mainline/arch/generic/lisp/runtime/gen-r18-hash-table.lisp#1 $
;;;
;;; Description:
...
(macrolet (
  ...
  (with-slot ((htb hash-code) &body decl*-form*)
    `(let* ((vec     (ref hash-table vector ,htb))
            ...
            (runner start)
            (rest   (svref vec 0)) )
          (declare (type sequence-index rest runner))
  ...
  (defun gethash/eq (key htb &optional default)
    (htb-get eq sxhash/eql) )

  (defun (setf gethash/eq) (val key htb &optional default)
        (declare (ignore default))
    (htb-set eq sxhash/eql) )

  (defun remhash/eq (key htb)
    (htb-rem eq sxhash/eq) )
...
;;;; 18.2.7 hash-table-rehash-size
(defun cl:hash-table-rehash-size (htb)
    (declare (values (float (1.0) *)))
  (ref hash-table rehash-size htb) )
...
;;; FIXME 2007-03-13: NYI: integer rehash-size.
(defun cl:make-hash-table (&key (test             'eql)
    ...
    (declare (type (or (integer 1 *) (float (1.0) *)) rehash-size))
    (declare (type (real 0 1) rehash-threshold))
  (let ((test (if (functionp test) (function-name test) test))
        (rehash-size
          (typecase rehash-size
            (single-float rehash-size)
            (double-float (float rehash-size))
            (integer 1.3) ) ))
      (declare (type (member eq eql equal equalp) test))
