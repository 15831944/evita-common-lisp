...
;;;
;;; @(#)$Id: //proj/evcl3/mainline/lisp/devel3/d03-eval.lisp#4 $
;;;
;;; Description:
...
;;;; 3.8.8 compiler-macro-function
(defun cl:compiler-macro-function (name &optional env)
    (declare (type function-name name))
    (declare (values (or null function)))
  (cdr (assoc :compiler-macro
...
(defun cl:eval (form)
  (labels (
    ...
    (compile-to-function (form)
      (or (compile-form form)
    ...
    (eval-compound-form (form)
      (let ((operator (first form)))
        (multiple-value-bind (kind local-p alist)
            (function-information operator si::*environment*)
          (when local-p
          ...
          (ecase kind
            ...
            ((:special-operator)
              (case operator
                ...
                (otherwise
                  (funcall (compile-to-function form)) )) )) ) ) )
        ...
        (typecase fname
          ((or symbol (cons (eql setf) (cons symbol null)))
            (multiple-value-bind (kind local-p)
                (function-information fname si::*environment*)
              (when local-p
      ...
      (let ((runner (rest form))
        ...
        (loop
          ...
          (let ((name (pop runner)))
            ...
            (multiple-value-bind (kind local-p alist)
                (variable-information name si::*environment*)
    ...
    (eval-symbol (symbol)
        ...
        (declare (values t))
      (multiple-value-bind (kind local-p alist)
          (variable-information symbol si::*environment*)
        (when local-p
